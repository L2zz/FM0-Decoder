...
if __name__ == "__main__":
    ...
    try:
        ...
        netowrk.restore_model(model_name)
        ...
        for file_name in file_name_list:
            try:
                ...
                success, success_bit, ber = test_set(
                    file_name, netowrk.test_model(np.array(input)), answer)

                print("\n\tTEST RESULT:\t" + str(success) +
                      " / " + str(len(input)), end=" ")
    ...
    try:
        ...
        print("\n\n\n\t\t***** SUMMARY *****")
        print("\tTEST RESULT:\t" + str(tot_success) +
              " / " + str(tot_file * test_size), end=" ")
        print(
            "(" + str(round(100 * (float(tot_success) / (tot_file * test_size)), 2)) + "%)")
        print("\tBER:\t\t" + str(round(100 * ber, 2)) + "%")
        print("\t\t*****\t*****\t*****")
        tot_time.print()
        print("\t\t*****\t*****\t*****")
