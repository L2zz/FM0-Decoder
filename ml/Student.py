...
class Student(tf.keras.Model):
    ...
    def build_model(self):
        try:
            ...
            x = Dense(6700, name="student_1")(input_layer)
            x = BatchNormalization(name="student_batch_1")(x)
            x = Activation('relu', name="student_activation_1")(x)

            x = Dense(1340, name="student_2")(x)
            x = BatchNormalization(name="student_batch_2")(x)
            x = Activation('relu', name="student_activation_2")(x)

            decoded = Dense(268, name="student_3")(x)
