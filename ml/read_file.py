...
def read_data_file(file_name, rn_idx):
    try:
        ...
        file_databit = open(enc256_path + file_name + "_databit_enc256", "r")
        signal = []
        databit = []

        count = 0
        for idx in tqdm(range(file_num), desc=file_name, ncols=100, unit=" signal"):
